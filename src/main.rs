        ...
        for (_, event) in glfw::flush_messages(&events) {
            match event {
                ...
                glfw::WindowEvent::Key(key  , _code, action, _modifiers) => match key {
                    ...
                    glfw::Key::Space =>  { if action == glfw::Action::Press { player.move_direction(Vector3::new(0.0, 2.0, 0.0)) } else if action == glfw::Action::Release { /*player.stop_move_direction(Vector3::new(0.0, 2.0, 0.0))*/ } },
